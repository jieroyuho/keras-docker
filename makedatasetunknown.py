...
###############################################################################################################################################
# if projectname='iscxp1testbyte', 
#   table name is "iscxp1testbyte", and field is timestamp,sip,dip,byte
#   and datatableindex is "iscxp1testbyte_sip2dipstate", and field is sip,dip,timedistance(sec) 
#
# if raw data is from bro, and bro table name is test,you can create specific format by follow sql
#    create table iscxp1testbyte as 
#    select UNIX_TIMESTAMP( STR_TO_DATE(ts,'%Y-%M-%d_%H:%i:%s'))as timestamp,`id.orig_h` as sip,`id.resp_h` as dip,orig_ip_bytes as byte from test
#
################################################################################################################################################
...
def zerofillfromtable_v2(sql,sip):
 beforet=0
 listsumsbyte=list()
...
def checksippoweron(s,sip):
  ...
  sql='select * from '+iptable+' where hm1 ="'+s+'" limit 1'
  cursor.execute(sql)
...
class collectseg:
 ...
 def iplistcut_v2(self,range,listsumsbyteall):
  listsegip=list()
  pic=range
  reductionrate=range/1800
  valuestart=0
...
#db = pymysql.connect(host=mysqlurl, user=mysqlid, passwd=mysqlkey, db=database)
...
print('total',len(listdip))
print('filte total',len(listdipfilte))
...
for sip in listsip:
  iptable='sip_'+sip.replace('.','_')
  sql="CREATE TABLE IF NOT EXISTS  "+iptable+" AS \
    SELECT FROM_UNIXTIME(trim(timestamp),'%Y-%m-%d %H:%i')AS hm1,SUM(`byte`) AS sumsbyte FROM  `"+datatable+"` WHERE  `sip` =  '"+ip+"'  GROUP BY hm1 "
  print sql
  cursor.execute(sql)
"""
...
for table in listtable:
 ...
 listsumsbyte=zerofillfromtable(sql)
